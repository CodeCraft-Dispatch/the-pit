...
- `runtime/kernel/deterministic-tick-loop.mjs` is the executable reference model
  for fixed tick advancement, semantic event sequencing, snapshots, diagnostics,
  and replay behavior.
- `runtime/kernel/command-dispatcher.mjs` owns command normalization, queue
  ordering, capability-gated command dispatch, and command-family handler
  routing.
...
- `runtime/kernel/wasm/command-dispatcher-core.mjs` is the staged Wasm-facing
  command-dispatcher slice. It proves the gated command queue metadata boundary
  without prematurely moving semantic routing into raw Wasm memory.
...
- Wasm command-dispatcher core: minimal boot/queue-metadata capability proof
...
- lawful degradation when command dispatcher is explicitly disabled;
- snapshot restore and incompatible capability rejection;
- unsafe command identifier rejection.

The command-dispatcher slice is covered by executable tests for:
