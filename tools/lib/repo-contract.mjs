...
export function validateRepoContract(rootDir = process.cwd()) {
  ...
  if (existsSync(kbReadmePath)) {
    ...
    for (const requiredReference of [
      ...
      if (!kbReadme.includes(requiredReference)) {
        errors.push(`knowledge base README must reference ${requiredReference}`);
      }
    }
  }
  ...
  if (existsSync(specificationsDir)) {
    ...
    const featureCount = entries.filter((entry) => typeof entry === 'string' && entry.endsWith('.feature')).length;
    if (featureCount === 0) {
      errors.push('specifications directory must contain at least one .feature file');
    }
  }
