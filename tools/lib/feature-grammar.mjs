...
export function validateFeatureDocument(source, options = {}) {
  ...
  for (const scenario of document.scenarios) {
    ...
    if (scenario.given === 0) {
      errors.push(
        `${path}: scenario "${scenario.name}" is missing a Given step`,
      );
    }

    if (scenario.when === 0) {
      errors.push(
        `${path}: scenario "${scenario.name}" is missing a When step`,
      );
    }

    if (scenario.then === 0) {
      errors.push(
        `${path}: scenario "${scenario.name}" is missing a Then step`,
      );
    }
  }
